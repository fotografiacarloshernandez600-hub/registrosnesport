...
export type AdminRegistration={id:string;event_id:string;folio:string|null;full_name:string;email:string;phone:string;category:string;shirt_size:string;payment_status:string;receipt_key:string;race_time_ms:number|null;kit_delivered_at:string|null;kit_delivered_by:string|null;created_at:string};
...
export default async function Dashboard(){
  ...
  try{
    [rows,events,portfolio,quotes]=await Promise.all([
      supabaseSelect<AdminRegistration>("registrations","select=id,event_id,folio,full_name,email,phone,category,shirt_size,payment_status,receipt_key,race_time_ms,kit_delivered_at,kit_delivered_by,created_at&order=created_at.desc&limit=1000"),
